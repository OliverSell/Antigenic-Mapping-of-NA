---
title: "Antigenic mapping report"
author: "Magnus Hilarius Ohlin Jepsen and Oliver Sellberg"
date: today
date-format: "DD/MM/YYYY"

format:
  pdf:
    toc: true
    number-sections: true
    colorlinks: true
---

# Antigenic mapping
```{r setup, include=FALSE}
source("../R/99_proj_func.R")

library("knitr")
library("readr")
library("dplyr")
library("kableExtra")

table_info <- read_tsv(file = "../data/table_info.tsv")
titer_clean <- read_tsv(file = "../data/titer_clean.tsv")
virus_type <- table_info$virus_type[1]
ag_names <- read_tsv(file = "../data/ag_names.tsv")
attr_table_ag <- read_tsv(file = "../data/attr_table_ag.tsv")
attr_table_sr <- read_tsv(file = "../data/attr_table_sr.tsv")

# Base path and initial folder name components
base_path <- "../results"
virus_in_use <- table_info |> pull(virus_type)         # pulling virus type from table_info
folder_prefix <- paste0("ac_map_", virus_in_use, "_")  # Concatenates the base folder name with the virus type
today_date <- format(Sys.Date(), "%Y-%m-%d")

latest_folder_path <- get_latest_version_folder(base_path, folder_prefix, today_date)

```

## Data information

### **First 5 rows and columns of titer table**
```{r, echo = FALSE, warning = FALSE, message = FALSE}
# Apply rounding to both numeric and character columns appropriately
titer_clean |>
  head(5) |>
  select(1:5) |>
  mutate(
    across(where(is.numeric), ~ round(., digits = 2)),   # Round numeric columns
    across(where(is.character), convert_and_round_char)  # Convert and round character columns conditionally
  ) |>
  kable()

```

### **Information about the dataset**
```{r, echo = FALSE}
knitr::kable(table_info)
```

### **Antigen attribute table**
First 5 rows of the antigen attribute table
```{r, echo = FALSE}
attr_table_ag |>
  head(5) |>
  kable("latex", booktabs = TRUE, escape = FALSE) |>
  kable_styling(full_width = FALSE) |>
  # Column size
  column_spec(1, width = "2cm") |>
  column_spec(2, width = "4.5cm") |>
  column_spec(3, width = "2cm") |>
  column_spec(4, width = "2cm") |>
  column_spec(5, width = "1.5cm")
```   

### **Serum attribute table**
First 5 rows of the serum attribute table
```{r, echo = FALSE}
attr_table_sr |> 
  head(5) |> 
  kable()
```

\vspace{10pt}

## Mapping

### Antigenic map - colored by type - with legend
```{r, echo = FALSE}
file_path_type_legend <- file.path(latest_folder_path, 
                                   paste0("ac_map_", 
                                          virus_in_use, "_", 
                                          today_date, ".jpg"))
# Include image
knitr::include_graphics(file_path_type_legend)
```

### Antigenic map - colored by type - with labels
```{r, echo = FALSE}
# Include graphic for antigenic map colored by type - with labels
file_path_type_labels <- file.path(latest_folder_path, 
                                   paste0("ac_map_", 
                                          virus_in_use, "_", 
                                          today_date, "_label.jpg"))
# Include image
knitr::include_graphics(file_path_type_labels)
```

### Antigenic map - colored by antigenic group - with legend

```{r, echo = FALSE}
# Include graphic for antigenic map colored by antigenic group - with legend
file_path_group_legend <- file.path(latest_folder_path, 
                                    paste0("ac_map_", 
                                           virus_in_use, "_", 
                                           today_date, "_grouped.jpg"))
# Include image
knitr::include_graphics(file_path_group_legend)
```

### Antigenic map - colored by antigenic group - with labels
```{r, echo = FALSE}
# Include graphic for antigenic map colored by antigenic group - with labels
file_path_group_labels <- file.path(latest_folder_path, 
                                    paste0("ac_map_", 
                                           virus_in_use, "_",
                                           today_date, "_grouped_label.jpg"))
# Include image
knitr::include_graphics(file_path_group_labels)
```

### Antigenic map blank
```{r, echo = FALSE}
# Include graphic for blank antigenic map
file_path_blank <- file.path(latest_folder_path, 
                             paste0("ac_map_", 
                                    virus_in_use, "_", 
                                    today_date, "_plain.jpg"))
# Include image
knitr::include_graphics(file_path_blank)
```

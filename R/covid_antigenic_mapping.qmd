...
ggplot(dd) +
  geom_point(aes(x = x, 
                 ...
                 shape = type, 
                 size = type)) +
  scale_shape_manual(values = c("AG" = 16, "SR" = 0)) + 
  scale_size_manual(values = c("AG" = 6, "SR" = 2)) + 
  ...
  theme_minimal() +
  labs(title = "Antigenic Map of COVID Variants",
       x = "Dimension 1", y = "Dimension 2",
       color = "Antigenic group", shape = "Type", size = "Type") +
...
x_range <- range(dd$x, na.rm = TRUE)
y_range <- range(dd$y, na.rm = TRUE)

# Optionally, expand the range slightly to ensure the border doesn't sit too close to the data points
expand_margin <- 0.1
x_range <- x_range + c(-expand_margin, expand_margin)
y_range <- y_range + c(-expand_margin, expand_margin)
...
ggplot(dd) +
  ...
  coord_cartesian(xlim = x_range, 
                  ylim = y_range) # Adjust plot limits to match the expanded range
```

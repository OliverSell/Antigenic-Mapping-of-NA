...
dd <- dd |> mutate(serum_group = attr_table$"Serum passages:"[match(dd$serum, attr_table$"Serum IDs:")])
...
colors_for_SR <- setNames(viridis::viridis(length(unique_SR), option = "viridis"), unique_SR) #heat.colors(n = length(unique_SR), alpha = 0.9, rev = FALSE)
...
ggplot(dd) +
  ...
  labs(title = "Antigenic map of SARS-CoV-2 variants",
       subtitle = "Neutralization titers analyzed via pseudotype virus assay",
       x = "MSD-1", 
       y = "MSD-2",
  ...
  theme(
    ...
    panel.border = element_rect(colour = "darkgrey", 
                                fill=NA, 
                                size=1.5),
    plot.caption = element_text(hjust = 1, vjust = 1, margin = margin(t = 1, unit = "pt"), size = 12, color = "darkgrey", face = "italic"),

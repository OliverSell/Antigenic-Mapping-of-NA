...
base_map +
  geom_mark_ellipse(
    data = dd_multiple_entries,
    aes(x = V1, y = V2, group = antigenic_group, fill = antigenic_group),
    expand = unit(0.05, 'npc'),
    alpha = 0.1
  ) +
  geom_point(data = filter(dd, type == "AG"), 
             aes(x = V1, y = V2, fill = antigenic_group), 
...
ag_df$cluster <- clustering_result$cluster


num_clusters <- max(ag_df$cluster[ag_df$cluster > 0])

cluster_colors <- viridis(num_clusters, option = "mako")

names(cluster_colors) <- as.character(paste0("cluster",seq_len(num_clusters)))

combined_colors <- c(cluster_colors, colors_for_AG)

base_map + 
  geom_mark_ellipse(
    data = filter(ag_df, cluster > 0),  #  type == "AG",
    aes(x = V1, y = V2, group = factor(cluster), color = factor(cluster)),
    fill = NA, 
    expand = unit(0.05, 'npc'), alpha = 0.5, size = 1
  ) +
  geom_point(data = filter(dd, type == "AG"), 
             aes(x = V1, y = V2, fill = antigenic_group), 
             shape = 21, size = 6, color = "black") +
...
ggsave(
  plot = ag_map_5,
  filename = paste0(base_filename,"_einstein", ".jpg"),
  path = folder_name,
  device = "jpg",
  width = 20,
  height = 12,

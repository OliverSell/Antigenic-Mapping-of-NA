...
base_map <- ggplot(dd) +
  ...
  scale_color_manual(values = combined_colors) +
   scale_x_continuous(breaks = seq(from = floor(min(dd$V1)), 
                                  to = ceiling(max(dd$V1)), by = 1)) +
  scale_y_continuous(breaks = seq(from = floor(min(dd$V2)), 
                                  to = ceiling(max(dd$V2)), by = 1)) +
  theme_minimal() +
